...
// task1. Массив скиллов (поле skills) всех людей,
// не должно быть повторяющихся скиллов,
// так же они должны быть отсортированы по алфавиту;

let skills = [];
input.forEach(function(e) {
  e.skills.forEach(function(el) {
    skills.push(el.toLowerCase());
  });
});
...
let names = [];
// - сначала отсортируем массив объектов по количетсву друзей
let tmp = input.sort(
  function(a,b) {
    return b.friends.length - a.friends.length;
  });

  // - потом достанем из каждого имя в новый массив
...
let friends = [];
input.forEach(function(e) {
  e.friends.forEach(function(el) {
    friends.push(el.name);
  });
});

friends.forEach(function(e) {
  let index = friends.indexOf(e);

  if (friends.indexOf(e, index +1) > 0) {
    let indexRepeat = friends.indexOf(e, index +1);
